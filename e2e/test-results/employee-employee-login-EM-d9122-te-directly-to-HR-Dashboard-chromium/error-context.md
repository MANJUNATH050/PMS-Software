...
- Name: employee\employee-login.spec.ts >> EMP001 - Unified Login Page & Automatic Role Routing >> EMP001-07 Automatic role-based routing: HR credentials navigate directly to HR Dashboard
- Location: tests\employee\employee-login.spec.ts:129:7
...
Expected pattern: /\/hr\/dashboard/
Received string:  "http://localhost/login"
Timeout: 5000ms

Call log:
  - Expect "toHaveURL" with timeout 5000ms
    14 × locator resolved to <html lang="en">…</html>
       - unexpected value "http://localhost/login"
...
- textbox "Enter your email address": hr@aseuro.com
- textbox "Enter your password": Hr@12345
- button "Show"
...
```ts
  ...
  132 |     await page.fill('#password', 'Hr@12345');
  133 |     await page.click('button[type="submit"]');
  134 | 
> 135 |     await expect(page).toHaveURL(/\/hr\/dashboard/);
      |                        ^ Error: expect(page).toHaveURL(expected) failed
  136 |     await expect(page.locator('text=HR Administration Dashboard')).toBeVisible();
  137 |   });
  138 | 
  139 |   test('EMP001-08 Automatic role-based routing: Manager credentials navigate directly to Manager Dashboard', async ({ page }) => {
  140 |     await page.goto('/login');
  141 |     await page.fill('#email', 'manager@aseuro.com');
  142 |     await page.fill('#password', 'password');
  143 |     await page.click('button[type="submit"]');
  144 | 
  145 |     await expect(page).toHaveURL(/\/manager\/dashboard/);
  146 |     await expect(page.locator('text=Welcome back, Alice Smith!').or(page.locator('text=Direct Reports')).first()).toBeVisible();
  147 |   });
  148 | });
  149 | 
  150 | 
