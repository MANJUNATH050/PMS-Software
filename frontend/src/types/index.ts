...
export interface Kpi {
  ...
  ratingStatus: 'DRAFT' | 'SUBMITTED' | 'COMPLETED' | 'PENDING' | 'FINALIZED' | 'IN_PROGRESS' | string;
}
...
export interface KpiMasterItem {
  ...
  description: string;
  weightage: number;
  applicableFor?: 'Employee' | 'Manager' | 'Both Employee & Manager' | string;
  kpiCategory?: 'ROLE_KPI' | 'HR_REVIEW_KPI' | string;
  status: string;
}
...
export interface EmployeeLifecycleData {
  ...
  roleKpis?: LifecycleKpiDetail[];
  hrReviewKpis?: LifecycleKpiDetail[];
  employeeSelfScore?: number | null;
  managerWeightedScore?: number | null;
  hrWeightedScore?: number | null;
...
export interface HrSaveRatingsPayload {
  ratings: Array<{
    kpiId: number;
    selfRating?: number;
    selfComment?: string;
