...
export const hrApi = {
  ...
  getDepartments: async (): Promise<string[]> => {
    const response = await apiClient.get<string[]>('/api/hr/departments');
    return response.data;
  },
  ...
  updateEmployee: async (id: number, data: UpdateEmployeePayload): Promise<EmployeeRecord> => {
    const response = await apiClient.put<EmployeeRecord>(`/api/hr/employees/${id}`, data);
    return response.data;
  },

  getKpiMasterList: async (designation?: string, category?: string): Promise<KpiMasterItem[]> => {
  ...
  createKpi: async (data: { designation?: string; kpiName: string; description: string; weightage: number; applicableFor?: string; kpiCategory?: string }): Promise<KpiMasterItem> => {
    const response = await apiClient.post<KpiMasterItem>('/api/hr/kpis', data);
    return response.data;
  },

  updateKpi: async (id: number, data: { kpiName: string; description: string; weightage: number; applicableFor?: string; kpiCategory?: string; status?: string }): Promise<KpiMasterItem> => {
  ...
  saveHrRatings: async (assignmentId: number, data: { ratings: Array<{ kpiId: number; hrRating?: number; comments?: string }>; hrComments?: string }): Promise<{ message: string; assignmentId: number }> => {
    const response = await apiClient.put<{ message: string; assignmentId: number }>(`/api/hr/lifecycle/${assignmentId}/ratings`, data);
    return response.data;
  },

  finalizePms: async (assignmentId: number, data: { overallScore?: number; performanceGrade?: string; hrComments?: string }): Promise<{ message: string; finalScore: number; grade: string }> => {
